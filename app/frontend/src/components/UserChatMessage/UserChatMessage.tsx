...
export const UserChatMessage = ({ message, approach }: Props) => {
    return (
        <div className={approach == Approaches.GPTDirect ? styles.containerUngrounded : styles.container}>
            <div className={approach == Approaches.Introduction ? styles.messageIntroduction : approach == Approaches.ChatWebRetrieveRead ? styles.messageweb : (approach == Approaches.ReadRetrieveRead || approach == Approaches.DecisionProposal || approach == Approaches.DocumentSummary ) ? styles.messagework : approach == Approaches.GPTDirect ? styles.messageungrounded : styles.messagecompare}>
                {
                approach == Approaches.ReadRetrieveRead ? 
                    <span style={{ marginRight: '10px' }}><BuildingMultiple20Filled primaryFill={"rgba(255, 255, 225, 1)"}/></span> : 
                approach == Approaches.Introduction ? 
                ...
                <span className={approach == Approaches.GPTDirect ? styles.userMessageUngrounded : approach == Approaches.Introduction ? styles.userMessageIntroduction : styles.userMessage}>{message}</span>
            </div>
        </div>
    );
};
