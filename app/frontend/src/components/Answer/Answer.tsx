...
import { Approaches, ChatResponse, getCitationFilePath, ChatMode, translateText } from "../../api";
...
export const Answer = ({
    ...
    const createDocument = (lines: string[]) => {
    ...
    const onDownloadTranslatedClick = async (answerHtml: string) => {
        document.body.style.cursor = "wait";
        ...
        const doc = createDocument(translatedLines);
        
        Packer.toBlob(doc).then((blob) => {
            // saveAs(blob, t("NameTemplateTranslated"));
            // console.log("Prijedlog odluke uspješno preuzet");
            ...
            a.href = url;
            a.download = t("Answer.NameTemplateTranslated");
    ...
    const saveConversationToLocal = (korisnikInfo: string) => {
        const blob = new Blob([korisnikInfo], { type: "text/plain" });
        const url = URL.createObjectURL(blob);
        
        const timestamp = new Date().toISOString().replace(/[:.-]/g, "").slice(0, 15);
        const fileName = `MoMo-${timestamp}.txt`;
        
        const link = document.createElement("a");
        link.href = url;
        link.download = fileName;  // The filename the browser will save
    ...
    return (
        ...
        <Stack className={`${(answer.approach == Approaches.ReadRetrieveRead || answer.approach == Approaches.DocumentSummary || answer.approach == Approaches.DecisionProposal || answer.approach == Approaches.OdlukeOdbora || answer.approach == Approaches.CreditApproval) ? styles.answerContainerWork :
            ...
            {(parsedAnswer.approach == Approaches.DecisionProposal && !!parsedAnswer.work_citations.length) && (
                <Stack.Item>
                    <Stack horizontal wrap tokens={{ childrenGap: 5 }}>
                        <div className={styles.downloadFileLA} onClick={() => onDownloadTranslatedClick(parsedAnswer.answerHtml)}> {t("Answer.TranslateDecision")} </div>
                        <div className={styles.downloadFileLA} onClick={() => onDownloadClick(parsedAnswer.answerHtml)}> {t("Answer.DownloadDecision")} </div>
                        {/* <div className={styles.downloadFileLA} onClick={() => onTranslateClick(parsedAnswer.answerHtml)}> Varijanta B</div> */}
                    </Stack>
                </Stack.Item>
            )}
