...
const ElevenLabsMain = () => {
  ...
  const getSystemPrompt = (salesType: salesType): string => {
   if (salesType === 'Paket') 
      return systemPromptPaket;
   else
      return systemPromptKredit;
  }

  const systemPromptKredit = `
...
Želim [pauza 2 sekunde] Vas informirati o mogućnosti financiranja koje Vam može pomoći u otplati vaših troškova ili financiranju vaših budućih planova. [pauza].
...
Za iznose kredita veće od 20 tisuća eura ugovaranje također možete započeti na m-zabi ili putem video sastanka, ali je potrebno doći u poslovnicu radi dovršetka procesa.
...
DEFINICIJA;
[pauza] 3000 ms šutnje.
`
//   const systemPromptKredit = `# Personality
...
`;
  ...
  useEffect(() => {
        ...
        console.log(`startConversation - Name: ${nameParam}, sales type: ${salesType}`);
